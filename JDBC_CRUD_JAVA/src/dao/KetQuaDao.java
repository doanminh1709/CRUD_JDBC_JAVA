...
public class KetQuaDao {
	...
	public List<KetQua> findAll() throws SQLException {
		...
		while(result.next()) {
			...
			p.setSinhVien(sv);
			mh.setTenMH(result.getString("tenmh"));
			mh.setMaMH(result.getString("mamh"));
			p.setMonHoc(mh);
			p.setDiemThi(result.getFloat("diemthi"));
	...
	public List<KetQua> find(String s) throws SQLException {
		List<KetQua> list = new ArrayList<>();
		String sql = "SELECT ketqua.masv, sinhvien.hoten, ketqua.mamh, monhoc.tenmh, ketqua.diemthi FROM ketqua INNER JOIN sinhvien ON sinhvien.MaSV = ketqua.MaSV INNER JOIN monhoc ON monhoc.mamh = ketqua.mamh WHERE hoten LIKE ?";
